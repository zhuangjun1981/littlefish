...
class Simulation(object):
    ...
    def initiate_simulation(self):
        ...
        if self._simulation_status == 0:
            ...
            self._simulation_histories.update(
                {
                    "food_pos_history": np.zeros(
                        (self._simulation_length, self._food_num, 2), dtype=np.uint32
                    )
                }
            )

            #  generate non-overlapping positions for all fish
            fish_start_positions = self._terrain.generate_fish_starting_position(
                len(self._fish_list)
            )
            ...
            for i, fish in enumerate(self._fish_list):
                ...
                simulation_history_curr_fish.update(
                    {"action_histories": fish._brain.generate_empty_action_histories()}
                )
                simulation_history_curr_fish.update(
                    {
                        "psp_waveforms": fish._brain.generate_empty_psp_waveforms(
                ...
                self._simulation_histories.update(
                    {fish.name: simulation_history_curr_fish}
                )
    ...
    def generating_fish_map(self, time_point, is_plot=False):
        ...
        if self._simulation_status in [2, 3, 4]:
            ...
            for fish_name, fish_history in self._simulation_histories.items():
                ...
                if curr_health > 0:
                    curr_row, curr_col = tuple(
                        fish_history["life_history"].loc[
                            time_point, ["pos_row", "pos_col"]
                        ]
                    )
                    ...
                    if (
                        (curr_row - 1 < 0)
                        or (curr_row + 2 > self.terrain_shape[0])
                        or (curr_col - 1 < 0)
                        or (curr_col + 2 > self.terrain_shape[1])
                    ):
                        print(
                            "Simulation: cannot plot fish {}. Position out of terrain boundary".format(
                                fish_name
                            )
                        )
                        return None
                    fish_map[
                        curr_row - 1 : curr_row + 2, curr_col - 1 : curr_col + 2
                    ] += 1
    ...
    def get_fish_health_status(self, t_point, verbose=False):
        ...
        for fish_n in self.fish_names:
            ...
            if (
                fish_life_his.loc[t_point, "pos_row"] == 0
                and fish_life_his.loc[t_point, "pos_col"] == 0
            ):
                raise RuntimeError(
                    "Simulation: Try to get fish health status at an unsimulated time point."
                )

            health_status.loc[fish_n, "health"] = float(
                fish_life_his.loc[t_point, "health"]
            )
    ...
    def _is_dead(self, fish_n, t_point):
        ...
        if (
            self._simulation_histories[fish_n]["life_history"].loc[t_point, "pos_row"]
            == 0
            and self._simulation_histories[fish_n]["life_history"].loc[
                t_point, "pos_col"
            ]
            == 0
        ):
        ...
        return (
            self._simulation_histories[fish_n]["life_history"].loc[t_point, "health"]
            <= 0.0
        )

    def _get_fish_status(self, t_point, fish_history):
        curr_position = [
            int(fish_history["life_history"].loc[t_point, "pos_row"]),
    ...
    def _move_fish(self, curr_fish, curr_t, curr_pos, curr_health, movement_attempt):
        ...
        if np.array_equal(movement_attempt, np.array([0, 0], np.int8)):
            ...
            if new_pos != curr_pos:
                msg += "Time: {:08d}; Fish: {}; Health: {:3.4f}; move from [{:3d}, {:3d}] to [{:3d}, {:3d}].".format(
                    curr_t,
                    curr_fish.name,
                    curr_health,
                    curr_pos[0],
    ...
    def run(self, verbose=1):
        ...
        if self._simulation_status == 2:
            ...
            if verbose > 1:
                curr_msg = "\nstart of simulation. start time: {}".format(
            ...
            while len(alive_fish_list) > 0 and curr_t < self.simulation_length:
                ...
                if (verbose > 1) and (self._simulation_length > 100):
                    if curr_t // (self.simulation_length // 10) > curr_progress:
                        curr_msg = "\n{:09.2f} second: {:2d} %".format(
                            time.time() - t0,
                ...
                curr_food_positions = self._terrain.update_food_map(
                    self.food_num, self._food_map
                )
                ...
                for curr_fish in alive_fish_list:  # loop through current live fish
                    ...
                    curr_position, curr_health = self._get_fish_status(
                        curr_t, curr_fish_history
                    )
                    ...
                    if (food_eated > 0) and (verbose > 1):
                        curr_msg = "Time: {:08d}; Fish: {}; eated {} food pellet(s). HP: {:3.4f} -> {:3.4f}.".format(
                            curr_t,
                            curr_fish.name,
                            food_eated,
                            curr_health,
                    ...
                    if (
                        curr_t < self.simulation_length - 1
                    ):  # if not at the end of simulation
                        # update fish's health at curr_t + 1
                        curr_fish_history["life_history"].loc[
                            curr_t + 1, "health"
                        ] = updated_health

                        if updated_health > 0:  # if not dead
                            new_pos, curr_msg = self._move_fish(
                                curr_fish,
                                curr_t,
                                curr_position,
                                curr_health,
                            ...
                            if curr_msg:
                                self._simulation_histories[curr_fish.name][
                                    "total_moves"
                                ] += 1
                                if verbose > 1:
                                    self._simulation_histories["message"] += (
                            ...
                            if verbose > 1:
                                curr_msg = "Time: {:08d}; Fish: {}; health: {:3.4f}; position:[{:3d},{:3d}]".format(
                                    curr_t + 1,
                                    curr_fish.name,
                                    updated_health,
                                    new_pos[0],
                            ...
                            if verbose > -1:
                                curr_msg = "Time: {:08d}; Fish: {} died. Total number of movements: {}.".format(
                                    curr_t,
                                    curr_fish.name,
                                    self._simulation_histories[curr_fish.name][
                                        "total_moves"
                ...
                if curr_t == self.simulation_length:
                    if verbose > 1:
                        ...
                        for sur_fish in alive_fish_list:
                            add_msg = "Survived fish: {}. Final health: {}. Total number of movements: {}".format(
                                sur_fish.name,
                                self._simulation_histories[sur_fish.name][
                                    "life_history"
                                ].loc[curr_t - 1, "health"],
                                self._simulation_histories[sur_fish.name][
                                    "total_moves"
    ...
    def save_log(self, log_folder, is_save_psp_waveforms=False):
        ...
        if self._simulation_status == 4:
            ...
            food_pos_dset = log_f.create_dataset(
                "food_pos_history", data=self._simulation_histories["food_pos_history"]
            )
            food_pos_dset.attrs[
                "data_format"
            ...
            for curr_fish in self._fish_list:
                ...
                if is_save_psp_waveforms:
                    ...
                    curr_fish_psp_wf_dset = curr_fish_sim_grp.create_dataset(
                        "psp_waveforms", data=curr_sim_history["psp_waveforms"]
                    )
                    curr_fish_psp_wf_dset.attrs[
                        "data_format"
                    ] = "neuron_ind x time_point"

                # save life history of fish
                curr_life_his = self._simulation_histories[curr_fish.name][
                    "life_history"
                ]
                curr_pos_arr = np.array(curr_life_his.loc[:, ["pos_row", "pos_col"]])
                curr_fish_pos_dset = curr_fish_sim_grp.create_dataset(
                    "position_history", data=curr_pos_arr
                )
