...
class TestEvolution(unittest.TestCase):
    ...
    def test_UniformMutation(self):
        ...
        assert um.get_value() >= 5.0
        assert um.get_value() < 10.0

        um2 = evo.UniformMutation(value_range=(6.0, 11.0), dtype="int")
    ...
    def test_mutate_neuron(self):
        ...
        um_n = evo.NeuronMutation(
            baseline_mutation=evo.UniformMutation(
                value_range=(0.0, 0.05), dtype="float"
            ),
            refractory_mutation=evo.UniformMutation(
                value_range=(1.3, 2.5), dtype="float"
            ),
        )
        mutated_neuron = evo.mutate_neuron(neuron=curr_neuron, neuron_mutation=um_n)
        assert mutated_neuron.get_baseline_rate() >= 0.0
    ...
    def test_mutate_connection(self):
        curr_connection = fi.Connection(
            latency=3, amplitude=0.001, rise_time=5, decay_time=10
        )
        um_con = evo.ConnectionMutation(
            latency_mutation=None,
            amplitude_mutation=evo.UniformMutation(
                value_range=(0.0001, 0.005), dtype="float"
            ),
            rise_time_mutation=None,
            decay_time_mutation=None,
        )
        mutated_connection = evo.mutate_connection(
            connection=curr_connection, connection_mutation=um_con
        )
    ...
    def test_mutate_brain(self):
        ...
        eye_mutation = evo.NeuronMutation(
            baseline_mutation=evo.UniformMutation((0.1, 0.5), "float"),
            refractory_mutation=evo.UniformMutation((5.0, 10.0), "float"),
        )
        neuron_mutation = evo.NeuronMutation(
            baseline_mutation=evo.UniformMutation((0.01, 0.03), "float"),
            refractory_mutation=None,
        )
        muscle_mutation = evo.NeuronMutation(
            baseline_mutation=None,
            refractory_mutation=evo.UniformMutation((13.0, 14.5), "float"),
        )
        connection_mutation = evo.ConnectionMutation(
            latency_mutation=evo.UniformMutation((15, 20), "int"),
        ...
        assert (
            mutated_brain.get_neurons().loc[0, "neuron"].get_refractory_period() >= 5.0
        )
        assert (
            mutated_brain.get_neurons().loc[0, "neuron"].get_refractory_period() < 10.0
        )
        ...
        assert (
            mutated_brain.get_neurons().loc[19, "neuron"].get_refractory_period() < 14.5
        )
    ...
    def test_populationevolution_calculate_offspring_num(self):
        ...
        pe = evo.PopulationEvolution(
            ...
            generation_digits_num=6,
        )
        life_thr, fishes = pe._calculate_offspring_num(
            generation_ind=4, turnover_rate=0.7
        )
        assert life_thr == 185
